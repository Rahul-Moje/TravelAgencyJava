...
public class UserProfileDao implements UserProfileRepository {
	...
	@Override
	public User retrieveUserDetail(String email) {
		...
		try {
			...
			while (resultSet.next()) {
				if (null != resultSet.getString("plan_name")) {
					user = new ProMember.ProMemberBuilder().withUserEmail(resultSet.getString("email_id"))
							.withUserName(resultSet.getString("username"))
							.withPlanName(PlanName.mapPlanName(resultSet.getString("plan_name")))
							.withDateOfPurchase(resultSet.getDate("date_of_purchase")).build();
				} else {
					user = new User.UserBuilder().withUserEmail(resultSet.getString("email_id"))
							.withUserName(resultSet.getString("username")).withIsProMember(false).build();
				}
			}
	...
	@Override
	public List<UserBooking> retrieveUserBookings(String email) {
		...
		try {
			...
			ResultSet resultSet = preparedStatement.executeQuery();
			while (resultSet.next()) {
				UserBooking.UserBookingBuilder builder = new UserBooking.UserBookingBuilder(resultSet.getInt("ID"))
						.withBookingDate(resultSet.getDate("BOOKINGDATE")).withSource(resultSet.getString("SOURCE"))
						...
						.withToFlightCode(resultSet.getString("TOFLIGHTCODE")).withToDate(resultSet.getDate("TODATE"))
